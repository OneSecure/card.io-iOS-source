...
- (void) setupGL {
  [self withContextDo:^{
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8_OES, (GLsizei)self->_size.width, (GLsizei)self->_size.height);
    ...
    float w = (float)self->_size.width;
    float h = (float)self->_size.height;
